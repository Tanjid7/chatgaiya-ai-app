...
<style>
    ...
    .app-subtitle { font-size: 1.1rem; color: #4B5563; text-align: center; margin-bottom: 2rem; }
    .result-box-bangla { background-color: #F0FDF4; border-left: 6px solid #10B981; padding: 20px; border-radius: 8px; margin-top: 15px; margin-bottom: 15px; }
    .result-box-english { background-color: #EFF6FF; border-left: 6px solid #3B82F6; padding: 20px; border-radius: 8px; margin-bottom: 20px; }
    .result-label { font-weight: 700; text-transform: uppercase; font-size: 0.8rem; color: #6B7280; }
    .result-text { font-size: 1.4rem; color: #111827; line-height: 1.4; }
    div.stButton > button[kind="primary"] { background-color: #1E3A8A !important; color: white !important; border: none !important; border-radius: 6px !important; font-weight: 600 !important; }
    .qnfl-badge { background-color: #f3f4f6; border: 1px solid #e5e7eb; padding: 8px 12px; border-radius: 6px; text-align: center; font-size: 0.85rem; color: #4b5563; font-weight: 500; margin-top: 20px; }
    .qnfl-badge span { font-weight: 800; color: #1E3A8A; letter-spacing: 1px; }
</style>
...
def load_models():
    try:
        # HERE IS YOUR NEW HOSTED MODEL!
        path_mt5 = "Tanjid7/Chatgaiya-mT5"
...
if translate_btn:
    if not input_text.strip():
        ...
        st.error("System Error: Neural Engine Offline.")
    else:
        with st.spinner("Executing Stage 1: Dialect Normalization..."):
            ...
            with torch.no_grad():
                outputs_mt5 = model_mt5.generate(inputs_mt5.input_ids, max_length=128, num_beams=5, repetition_penalty=2.5, early_stopping=True)
            bangla_translation = tokenizer_mt5.decode(outputs_mt5[0], skip_special_tokens=True)

        st.markdown(f'<div class="result-box-bangla"><div class="result-label">Standard Bangla Output</div><div class="result-text">{bangla_translation}</div></div>', unsafe_allow_html=True)
        ...
        if include_english:
            with st.spinner("Executing Stage 2: Global Synthesis..."):
                ...
                with torch.no_grad():
                    outputs_en = model_en.generate(inputs_en.input_ids, max_length=128, num_beams=4, early_stopping=True)
                english_translation = tokenizer_en.decode(outputs_en[0], skip_special_tokens=True)

            st.markdown(f'<div class="result-box-english"><div class="result-label">English Output</div><div class="result-text">{english_translation}</div></div>', unsafe_allow_html=True)
